...
export default function OrdineSimulatoPage() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (!ordiniRes.ok) {
          throw new Error(ordiniPayload.error || "Errore nel caricamento testata ordine");
        }
        if (!righeRes.ok) {
          throw new Error(righePayload.error || "Errore nel caricamento righe ordine");
        }

        const head = Array.isArray(ordiniPayload.data) ? ordiniPayload.data[0] ?? null : null;
        if (!head) {
          throw new Error("Ordine non trovato nel database locale");
        }
  ...
  const parsedLines = useMemo<OrderLine[]>(() => {
      ...
      <div className="mx-auto max-w-5xl rounded-[28px] border border-slate-300/80 bg-white/90 p-6 shadow-[0_24px_70px_rgba(15,23,42,0.12)] backdrop-blur-sm md:p-8">
        <header className="border-b border-slate-200 pb-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <h1 className="text-3xl font-bold tracking-tight text-slate-950">Ordine Simulato</h1>
              <p className="mt-1 text-sm text-slate-500">Documento simulato per verifica operativa</p>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-slate-50/80 px-4 py-3 text-right text-sm text-slate-600">
              <div>
                Numero: <span className="font-semibold text-slate-900">{numero || "-"}</span>
              </div>
              <div>
                Data: <span className="font-semibold text-slate-900">{dataDoc ? formatDate(dataDoc) : "-"}</span>
              </div>
              <div>
                Registro: <span className="font-semibold text-slate-900">{numReg || "-"}</span>
              </div>
            </div>
          </div>
        </header>

        {loading && <p className="py-10 text-sm text-slate-500">Caricamento dati ordine in corso...</p>}
        ...
        {!loading && !error && (
          <>
            <section className="grid gap-4 py-6 md:grid-cols-2">
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                <h2 className="text-xs font-semibold uppercase tracking-wide text-slate-500">Cliente/Fornitore</h2>
            ...
            <section className="ml-auto mt-6 w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="flex items-center justify-between border-t border-slate-200 pt-2 text-base">
                <span className="font-semibold text-slate-900">Totale ordine</span>
                <span className="font-bold text-slate-950">{formatMoney(totale)}</span>
              </div>
            </section>

            <p className="mt-6 text-xs text-slate-500">Documento simulato: verificare sempre il documento ufficiale del gestionale.</p>
          </>
        )}
      </div>
    </main>
